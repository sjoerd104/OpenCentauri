...
fn main() {
    ...
    let current_configs_count_1 =
        u32::from_le_bytes(bytes[0x00..0x04].try_into().unwrap()) as usize;
    let current_configs_count_2 =
        u32::from_le_bytes(bytes[0x04..0x08].try_into().unwrap()) as usize;
    ...
    match args.action {
        config::Commands::List => {
            println!(
                ...
                current_configs_count_1, max_configs_count
            );
