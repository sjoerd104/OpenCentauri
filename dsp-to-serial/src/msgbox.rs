...
fn get_ept_interface_by_name(rpmsg_endpoint_info: &RpmsgEndpointInfo) -> Option<PathBuf>
{
    ...
    let name_len = rpmsg_endpoint_info.name.iter().position(|&b| b == 0).unwrap();
...
impl MsgboxEndpoint
{
    pub fn new() -> Result<MsgboxEndpoint, ApplicationError>
    {
        ...
        wrap_ioctl_negative_invalid(unsafe { rpmsg_create_ept_ioctl(msgbox_fd_ctrl.as_raw_fd(), &ept_info)})?;
        let ept_interface = match get_ept_interface_by_name(&ept_info)
        {
            Some(ept_interface) => ept_interface,
            None => return Err(ApplicationError::UnknownError("Failed to find opened ept interface"))
        };
